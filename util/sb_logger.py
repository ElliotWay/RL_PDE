...
class HumanOutputFormat(KVWriter, SeqWriter):
    ...
    def _truncate(cls, string):
        return string[:30] + '...' if len(string) > 33 else string
...
class CSVOutputFormat(KVWriter):
    ...
    def writekvs(self, kvs):
        ...
        extra_keys = [key for key in kvs.keys() if key not in self.keys]
        # The original line changes key order:
