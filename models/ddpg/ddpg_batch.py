...
def softmaxize(tensor):
    """
    PDE
    Make the values along the last dimension add up to 1.
    Not actually softmax, but resembles it.
    Putting this here so we can change it quickly.
    """
    return tensor / np.sum(tensor, axis=-1)[..., None]
...
class DDPGBatch(OffPolicyRLModel):
    """
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.policy_tf = self.policy(self.sess, self.i_observation_space, self.i_action_space, 1, 1, None,
                                                 **self.policy_kwargs)

                    # Create target networks.
                    self.target_policy = self.policy(self.sess, self.i_observation_space, self.i_action_space, 1, 1, None,
                                                     **self.policy_kwargs)
                    ...
                    if self.param_noise is not None:
                        ...
                        self.param_noise_actor = self.policy(self.sess, self.i_observation_space, self.i_action_space, 1, 1,
                        ...
                        self.adaptive_param_noise_actor = self.policy(self.sess, self.i_observation_space,
                                                                      self.i_action_space, 1, 1, None,
                                                                      **self.policy_kwargs)
                    ...
                    self.actions = tf.placeholder(tf.float32, shape=(None,) + self.i_action_space.shape, name='actions')
                ...
                with tf.variable_scope("input_info", reuse=False):
                    ...
                    if self.full_tensorboard_log:
                        ...
                        if len(self.i_observation_space.shape) == 3 and self.i_observation_space.shape[0] in [1, 3, 4]:
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        print("Predict function not reimplemented for DDPGBatch.")
        sys.exit(0)
