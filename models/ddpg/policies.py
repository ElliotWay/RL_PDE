...
class FeedForwardPolicy(DDPGPolicy):
    ...
    def make_actor(self, obs=None, reuse=False, scope="pi"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            ac_flat_size = np.prod(self.ac_space.shape)
            output = tf.nn.tanh(tf.layers.dense(pi_h, ac_flat_size, name=scope,
                                                     kernel_initializer=tf.random_uniform_initializer(minval=-3e-3,
                                                                                                      maxval=3e-3)))
            self.policy = tf.reshape(output, (-1,) + self.ac_space.shape)
    ...
    def make_critic(self, obs=None, action=None, reuse=False, scope="qf"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for i, layer_size in enumerate(self.layers):
                ...
                if i == 0:
                    ac_flat_size = np.prod(self.ac_space.shape)
                    action = tf.reshape(action, (-1, ac_flat_size))
