...
class SACModel(BaselinesModel):
    ...
    def __init__(self, env, args):
        ...
        sac.learning_rate = get_schedule_fn(self.learning_rate)
    ...
    def train(self, s, a, r, s2, done):
        ...
        self.steps_seen += len(obs)
        step = self.steps_seen

        with SetVerbosity(sac.verbose), \
             TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                 as writer:

            self.replay_buffer.extend(s, a, r, s2, done)
            ...
            for grad_step in range(sac.gradient_steps):
                ...
                if not sac.replay_buffer.can_sample(self.batch_size) \
                   or step < self.learning_starts:
                    break
                self.n_updates += 1
            ...
            if step - self.prev_target_update > sac.target_update_interval:
                ...
                self.sess.run(sac.target_update_op)
